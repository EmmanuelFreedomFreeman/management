...
function Projet(props: any) {

    const [projetname, setProjetname] = useState('')
    const [projetnam, setProjetnam] = useState('')
    const addprojet = async () => {
        
        try {
          await addDoc(collection(db, "management"), {
            nomduprojet: projetname,temp:Date.now()
          });
          setProjetname('')
          //alert('inserted')
        } catch (error) {
    ...
    const updateprojet = async (id:any,projet:string) => {
      let proj = projetnam == '' ? projet : projetnam
      try {
                            ...
                            {props?.data.map((value:any,index:number)=>(
                              <tr className={value?.data?.nomduprojet?"border-b dark:border-neutral-500":'hidden'} key={index}>
                                ...
                                <td className="whitespace-nowrap px-6 py-4">
                                    <input type='text' placeholder={value?.data?.nomduprojet} onChange={(e)=> setProjetnam(e.target.value)}  />
                                </td>
                                <td className="whitespace-nowrap px-6 py-4">
                                  <p onClick={()=>updateprojet(value?.id,value?.data?.nomduprojet)} className='bg-green-300 p-2 cursor-pointer rounded-md hover:font-bold hover:text-xl'>UPDATE</p>
                                </td>
                                <td onClick={()=>deleteprojet(value?.id)} className={props?.user?.username!=''? "whitespace-nowrap px-6 py-4":'hidden'}>
